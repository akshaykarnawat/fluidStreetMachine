...
args = getResolvedOptions(
    sys.argv, ["JOB_NAME", "URL", "PORT", "USER", "PASSWORD", "QUERY", "DEST"]
)
sc = SparkContext()
...
_ = (
    ...
    .option(
        "url", args.get("URL")
    )  #'jdbc:mysql://database-1.cgqybx2c0nza.us-east-1.rds.amazonaws.com'
    .option("port", args.get("PORT"))  #'3306'
    .option("user", args.get("USER"))  #'admin'
    .option("password", args.get("PASSWORD"))  #'password123'
    .option(
        "dbtable", f"{args.get('QUERY')} AS t"
    )  #'(SELECT * FROM test.test_table LIMIT 10) AS t')
